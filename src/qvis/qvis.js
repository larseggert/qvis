...
decompress("__DATA_B64GZ__")
  .then((D) => {
    ...
    for (const [t, type, value, sid] of D.fcEvents || []) {
      ...
      if (sid === -1) {
        tvPush(
          type === "blocked" ? fcConnBlockedData : fcConnIncreasedData,
          t,
          bv,
        );
      } else {
    ...
    const ccLegEls = {};
    {
      ...
      el.querySelectorAll("[data-rg]").forEach((s) => {
        ccLegEls[s.dataset.rg] = s;
      });
    }
    ...
    function highlightRegion(t) {
      ...
      else if (findInterval(fcStreamIntervals, t))
        ...
        hooks: {
          drawSeries: [
            (u, si) => {
              ...
              const { left, top, width, height } = u.bbox;
              const r =
                ((u.series[si].points?.size || 5) / 2) * devicePixelRatio;
    ...
    function dedupAckLoss(candidates, u, cy) {
      ...
      for (const c of ackLoss) {
        ...
        const dy = Math.abs(
          (u.series[c.si].scale ? u.valToPos(c.v, u.series[c.si].scale) : 0) -
            cy,
        );
    ...
    function mkChart(
      ...
      if (linked) {
        ...
        for (const k of linked)
          if (yRanges[k]) mx = Math.max(mx, yRanges[k].max);
                ...
                (u) => {
                  ...
                  for (const { si, hi, v } of deduped) {
                    ...
                    if (isLossSeries) {
                      ...
                      html += `<div class="ip-frame">` + DD(N(pn));
                      if (myRange)
                        html += DD(`in ${R(myRange[0], myRange[1])}`);
                  ...
                  for (let i = 0; i < pts.length; i++) {
                    pts[i].style.display = shownSi.has(i + 1)
                      ? "block"
                      : "none";
      ...
      for (let si = 1; si < series.length; si++) {
        ...
        lbl.insertAdjacentHTML(
          "beforeend",
          swatch(s) +
            `<span class="ip-color" style="--c:${sColor(s)}">${H(s.label)}</span>`,
        );
        ...
        hooks: {
            ...
            (u) => {
              ...
              u.ctx.fillText = function (text, x, y, maxW) {
                for (let ai = 1; ai < u.axes.length; ai++) {
                  ...
                  const lbl =
                    typeof ax.label === "function" ? ax.label(u, ai) : ax.label;
                  if (lbl && text === lbl) {
          ...
          draw: [
            (u) => {
              ...
              const lbl =
                typeof ax.label === "function" ? ax.label(u, pos.ai) : ax.label;
              if (!lbl) return;
              const ctx = u.ctx;
              const r = devicePixelRatio;
              ctx.save();
              ctx.setTransform(pos.transform);
              ctx.font = ax.labelFont?.[0] || "bold 12px system-ui, sans-serif";
              ...
              let cx = pos.x,
                cy = pos.y;
              if (pos.textAlign === "start" || pos.textAlign === "left")
                cx += m.width / 2;
              else if (pos.textAlign === "end" || pos.textAlign === "right")
                cx -= m.width / 2;
              if (pos.textBaseline === "alphabetic") cy -= (asc - dsc) / 2;
              else if (pos.textBaseline === "top") cy += (asc + dsc) / 2;
              else if (pos.textBaseline === "bottom") cy -= (asc + dsc) / 2;
    ...
    function wrapPn(arr) {
      ...
      for (let i = 0; i < arr.length; i++) {
        ...
        const w = arr[i] % pnMod;
        if (i > 0 && arr[i - 1] != null && w < arr[i - 1] % pnMod)
          out[i] = null;
        else out[i] = w;
      }
      return out;
    ...
    if (D.lost.reordering_threshold) {
      ...
      for (const pn of D.lost.reordering_threshold[1]) {
        ...
        if (lo < gaps.length && pn >= gaps[lo].lo)
          lossGapInfo.set(pn, gaps[lo]);
      }
    }
    ...
    const panels = [
      ...
      },
      {
        ...
        series: [
          ...
          S("ssthresh", OI.vermillion, "bytes", {
            dash: [6, 3],
            paths: stepped,
          }),
    ...
    const isZoomed = (range, full) =>
      full > 0 && range < full * zoomResetThreshold;
    const applyRange = (ch, k, str) => {
      const r = decRange(str);
      if (r) ch.setScale(k, r);
    };
    const saveRange = (params, key, { min, max }, full) => {
      if (min != null && isZoomed(max - min, full))
        params.set(key, encRange(min, max));
    };
    const saveUrlState = (() => {
      let timer;
      return () => {
    ...
    for (const ch of charts) {
      ch.hooks.setScale ??= [];
      ch.hooks.setScale.push(saveUrlState);
    }
